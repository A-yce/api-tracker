<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        $stmt = $conn->prepare("INSERT INTO expenses (user_id, description, amount, time) VALUES (?, ?, ?, NOW())");
        $stmt->bind_param("sss", $id, $description, $amount);
    
        if ($stmt->execute()) {
            echo json_encode(['message' => 'Expense saved.']);
        } else {
            echo json_encode(['message' => 'Error saving expense: ' . $stmt->error]);
        }
